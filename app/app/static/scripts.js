...
$(document).ready(function() {
    ...
    originalBrowserOptions = Array.from(document.getElementById('browserAnalysisType').options);
    ...
    function updateFileListBasedOnExtension() {
        ...
        fetch(BASE_URL + '/api/list_filtered_files', {
            ...
            body: JSON.stringify({ session_id: sessionId, extension: selectedExtension })
        })
        .then(response => response.json())
        .then(response => {
            if (response.status === "success") {
                ...
                document.getElementById('analysisList').innerHTML = '<p>No files found.</p>';
            }
        })
        .catch(error => {
            console.error('Error fetching filtered files:', error);
            document.getElementById('analysisList').innerHTML = '<p>Error retrieving files.</p>';
        });
    }
    
    document.getElementById('fileTypeSelect').addEventListener('change', updateFileListBasedOnExtension);
    ...
    function selectPlants() {
        var plant = Array.from(document.querySelectorAll('#plant option:checked'))
    ...
    function adjustAnalysisOptions(single) {
        ...
        originalOptions.forEach(option => {
            if (option.value.endsWith(`_${filterType}`) || option.value.endsWith('_common')) {
    ...
    function adjustBrowserOptions(single, plants) {
        ...
        originalBrowserOptions.forEach(option => {
            ...
            const isFilterMatch = option.value.endsWith(`_${filterType}`) || option.value.endsWith('_common');
    ...
    function displayImages(figures) {
        ...
        figures.forEach(function(imageData) {
            var thumb = $('<a>').attr({
                ...
                'data-caption': 'Click the right or left side of the image to move to the next or previous image.'
            }).append(
                $('<img>')
    ...
    function startBrowser() {
        var initialSpeciesFilter = $('#plant option:selected').map(function() {
            return $(this).text();
        }).get();
        ...
        var speciesFilterParam = encodeURIComponent(JSON.stringify(initialSpeciesFilter));
        $('#browserIframe').attr('src', BASE_URL + '/api/browser?species_filter=' + speciesFilterParam);
    ...
    function startInfo() {
        ...
        var speciesFilterParam = encodeURIComponent(JSON.stringify(initialSpeciesFilter));
        $('#infoIframe').attr('src', BASE_URL + '/api/general_info?species_filter=' + speciesFilterParam);
    ...
    $('#runBrowserAnalysis').click(function() {
        ...
        var highlightList = $('#highlightListBrowser').val().split(',').map(item => item.trim()).filter(item => item.length > 0);
        ...
        if (nTopPercent === "" || !isNumeric(nTopPercent) || parseFloat(nTopPercent) < 0.1 || parseFloat(nTopPercent) > 100) {
            isValid = false;
            errorMessage += "Top percentage must be between 0.1 and 100.\n";
        }

        if (threshold === "" || !isNumeric(threshold) || parseFloat(threshold) < 0 || parseFloat(threshold) > 1) {
            isValid = false;
            errorMessage += "Threshold must be between 0 und 1.\n";
        }

        if (maxPval === "" || !isNumeric(maxPval) || parseFloat(maxPval) < 0 || parseFloat(maxPval) > 1) {
            isValid = false;
            errorMessage += "Max P-Value must be between 0 und 1.\n";
        }

        if (minFe === "" || !isNumeric(minFe) || parseFloat(minFe) < 0 || parseFloat(minFe) > 1) {
            isValid = false;
        ...
        if (minOrthos === "" || !isNumeric(minOrthos) || parseInt(minOrthos) < 0) {
            isValid = false;
            errorMessage += "Minimum Number of Orthogroups must be a positive number.\n";
        }
    ...
    $('#runAnalysis').click(function() {
        ...
        if (analysisType === "plot_correlation_network" || analysisType === "plot_co_expression_network") {
            requestData.threshold = parseFloat(threshold);
        ...
        if (newWindow) {
            ...
            if (result.plot_url) {
                if (result.plot_url.endsWith('.html')) {
                    console.log('Setting iFrame with URL:', result.plot_url + '?t=' + timestamp);
                    $(resultsDiv).html('<iframe id="' + frameID + '" scrolling="no" src="' + result.plot_url + '?t=' + timestamp + '" width="100%" height="600px" frameborder="0"></iframe>');
                } else {
                    console.log('Setting image with URL:', result.plot_url + '?t=' + timestamp);
                    $(resultsDiv).html('<img src="' + result.plot_url + '?t=' + timestamp + '" alt="Analysis Plot" width="100%" height="100%" />');
                }
            } else if (result.message) {
    ...
    function sendAnalysisRequest(analysisType, requestData, type, text) {
        ...
        startAnalysis(requestData).then(taskId => {
    ...
    $('#clearFilesButton').click(function() {
        ...
        fetch(`${BASE_URL}/api/cleanup_session_files`, {
            ...
            console.error('There was an issue cleaning up the session files:', error);
        });
    });
    ...
    function updateInputFields(){
        ...
        $("#generalHelpIcon").off('click').on('click', function() {
    ...
    function updateBrowserInputFields(){
        ...
        $("#browserHelpIcon").off('click').on('click', function() {
            $("#description_" + selectedAnalysis).toggle();
        });
    ...
    window.addEventListener("beforeunload", function () {
        ...
        const data = JSON.stringify({session_id: sessionStorage.getItem('session_id')});
    ...
    $('#plant').on('hidden.bs.select', function (e, clickedIndex, isSelected, previousValue) {
        $("#plant").prop("disabled", true);
        handleSelectionChange();
    }); 

    // Initialize button text based on the collapse state for non-Navbar buttons
    $('.toggle-button').not('.navbar-toggler').each(function() {
    ...
    $('.collapse').on('show.bs.collapse', function() {
        $('button.toggle-button[data-target="#' + this.id + '"]').not('.navbar-toggler').find('span').text('Hide');
    }).on('hide.bs.collapse', function() {
        $('button.toggle-button[data-target="#' + this.id + '"]').not('.navbar-toggler').find('span').text('Show');
    });
